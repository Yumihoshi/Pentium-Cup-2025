...
public class DemoManager : MonoBehaviour
{
    ...
    {
        text_fx_name.text =
            "[" + (index_fx + 1) + "] " + fx_prefabs[index_fx].name;
    }
    ...
    private void Update()
    {
        if (Input.GetMouseButtonDown(0))
        {
            ...
            ray_cast_hit = Physics2D.Raycast(
                new Vector2(ray.origin.x, ray.origin.y), new Vector2(0, 0));
            if (ray_cast_hit)
                switch (ray_cast_hit.transform.name)
                {
                    case "Wallpaper":
                        Instantiate(fx_prefabs[index_fx],
                            new Vector3(ray.origin.x, ray.origin.y, 0),
                            Quaternion.identity);
                        break;
                    case "UI-arrow-right":
                        ...
                        if (index_fx >= fx_prefabs.Length)
                            index_fx = 0;
                        text_fx_name.text = "[" + (index_fx + 1) + "] " +
                                            fx_prefabs[index_fx].name;
                        break;
                    case "UI-arrow-left":
                        ray_cast_hit.transform.SendMessage("Go");
                        index_fx--;
        ...
        if (Input.GetKeyDown("space"))
            Instantiate(fx_prefabs[index_fx], new Vector3(0, 0, 0),
                Quaternion.identity);
    }
}
